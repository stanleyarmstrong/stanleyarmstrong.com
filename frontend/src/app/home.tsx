...
export default function Home({ content }: { content: ContentData }) {
  ...
  return (
    ...
    <section className='fixed top-0 left-0 z-50 mt-3 w-full px-4 sm:px-10'>
      <nav className='relative flex items-center justify-between'>
          <p className='relative z-10 rounded-md bg-white/90 px-3 py-1.5 text-lg backdrop-blur-sm hover:text-teal-700 font-orbitron sm:text-2xl dark:bg-[#28303B]/90 dark:text-white dark:hover:text-teal-400'>
            <a href='#intro'>
              Stanley Armstrong
            </a>
          </p>
          ...
          <div className='w-full min-w-0 max-w-4xl md:flex-1'>
            <div className='w-full overflow-x-auto px-2 pb-2'>
              ...
              {content?.intro?.map((paragraph, index) => (
                index === 0 ? (
                  <h1 className='text-2xl font-semibold mt-5 hover:text-teal-700 dark:text-white dark:hover:text-teal-400' key={`intro-${index}`}>{paragraph}</h1>
                ) : (
            ...
            <div className='flex flex-col items-center mt-6 gap-2'>
              <button className='bg-teal-700 hover:bg-orange-500 text-white rounded-xl font-semibold dark:bg-teal-400 dark:hover:bg-yellow-300 px-5 py-2'>
                <a
                  href='https://calendar.app.google/romAGcoWuzukrfn76'
                  onClick={() => track('Book a Discovery Call', { location: 'intro' })}
                >
      ...
      <section id='services' className='mt-8 scroll-mt-20'>
        ...
        <div className='flex flex-col items-center mt-8 gap-2'>
          <button className='bg-teal-700 hover:bg-orange-500 text-white rounded-xl font-semibold dark:bg-teal-400 dark:hover:bg-yellow-300 px-6 py-2 text-xl'>
            <a
              href='https://calendar.app.google/romAGcoWuzukrfn76'
              onClick={() => track('Book a Discovery Call', { location: 'services' })}
            >
              Book a Discovery Call
            </a>
          </button>
